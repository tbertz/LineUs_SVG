...
IFButton b_up;
IFButton b_down;
IFLabel l_resolution;
IFTextField t_resstep;
IFButton b_upres;
IFButton b_downres;
IFLabel l_rotate;
...
void setup() {
  ...
  l_resolution = new IFLabel("Resolution", 20, 200);
  t_resstep = new IFTextField("Res Step", 65, 220, 30, "10");
  b_upres = new IFButton("-", 40, 220, 20, 20);
  b_downres = new IFButton("+", 20, 220, 20, 20);
  ...
  b_upres.addActionListener(this);
  b_downres.addActionListener(this);
...
void actionPerformed (GUIEvent event) {
  ...
  int t = 2;
  float restep = float(t_resstep.getValue());
  ...
  if (event.getSource() == b_upres && !plotting) { //increase res
    lines = true;
    resolution = resolution+restep;
  }
  if (event.getSource() == b_downres && !plotting) { //decrease res
    lines = true;
    resolution = resolution-restep;
  }
